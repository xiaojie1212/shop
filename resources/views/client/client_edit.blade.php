        ...
        <form action="{{route('admin.client.update')}}" method="post" class="form-horizontal">
            <div class="modal-body">
                ...
                {{--<div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <select class="select2" name="client_role" style="width: 85%">
                            @foreach($client_role as $k=>$v)
                                <option @if($client->id==$v->client_role) selected @endif value="{{$v->client_role}}">{{$v->role_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>--}}
